...
class MappingEventEntity:
  ...
  def to_dict(self):
    ...
    if self.connections:
      result['connections'] = self.connections # 5
    if self.links:
      result['links'] = self.links # 5
    if self.translation:
      result['translation'] = BuildingTranslation.expand_dict(self.translation) # 2
    if self.missing_telemetry_fields:
      result['missing_telemetry_fields'] = self.missing_telemetry_fields # 5
    return result
