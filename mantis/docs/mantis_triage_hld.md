...
The `TriagePipeline` ([pipeline.py](file:///usr/local/google/home/heykhyati/Projects/udmi_clone/udmi/mantis/src/triage/harness/pipeline.py)) is the primary orchestrator. It manages the execution of individual diagnostic stages sequentially, accumulates context, and resolves tools dynamically configured for each stage.
...
The `AsyncTriageEngine` ([engine.py](file:///usr/local/google/home/heykhyati/Projects/udmi_clone/udmi/mantis/src/triage/harness/engine.py)) drives the autonomous tool-calling loop, manages API retries, limits active concurrency via Semaphores, and applies critical guardrails to prevent token-count failures or API rate violations.
...
The `SemanticCache` ([cache.py](file:///usr/local/google/home/heykhyati/Projects/udmi_clone/udmi/mantis/src/triage/harness/config/cache.py)) acts as a localized vector similarity database. 
...
The harness provides a pre-built `ToolBelt` class ([tools.py](file:///usr/local/google/home/heykhyati/Projects/udmi_clone/udmi/mantis/src/triage/harness/tools.py)) exposing critical debugging APIs:
...
Expose a custom launcher script ([runner.py](file:///usr/local/google/home/heykhyati/Projects/udmi_clone/udmi/mantis/src/triage/impl/runner.py)) that:
