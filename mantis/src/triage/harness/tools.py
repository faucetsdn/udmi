...
class ToolBelt:
    ...
    def __init__(self, workspace_root: str, search_dirs: List[str] = None, exclude_dirs: List[str] = None, exclude_files: List[str] = None, include_files: List[str] = None):
    ...
    def grep_codebase(self, pattern: str) -> str:
        ...
        Uses ultra-fast 'git grep' as the primary strategy, falling back to standard grep if needed.
        """
        print(color_text("[Inspect Tool]:", BLUE, bold=True) + "\n" +
        ...
        for sdir in self.search_dirs:
            full_sdir = os.path.join(self.workspace_root, sdir)
            if not os.path.exists(full_sdir):
                continue

            # 1. Try ultra-fast 'git grep' first
            try:
                cmd = [
                    "git",
                    "grep",
                ...
                out = subprocess.check_output(cmd, cwd=self.workspace_root, text=True)
                if out.strip():
                    results.append(out.strip())
                continue  # Successfully searched this dir
            except subprocess.CalledProcessError as e:
                # git grep returns 1 if no matches found, which is fine.
                # Any other non-zero exit code (like 128) means we are not in a git repo or git failed.
                if e.returncode == 1:
                    continue
            except Exception:
                pass

            # 2. Try ultra-fast 'ripgrep' (rg) next if available
            import shutil
            if shutil.which("rg"):
                try:
                    ...
                    for edir in default_excludes.union(self.exclude_dirs):
            ...
            try:
                ...
                cmd.extend([pattern, sdir])
                out = subprocess.check_output(cmd, cwd=self.workspace_root,
                                              text=True)
                if out.strip():
                    results.append(out.strip())
        ...
        for sdir in self.search_dirs:
            ...
            try:
                cmd = [
                    "git",
                    "grep",
                    "-nIE",  # -E for extended regex matching
                    pattern,
                    "--",
                    sdir
                ]
            ...
            # 2. Try 'ripgrep' (rg) next if available
            import shutil
            if shutil.which("rg"):
                try:
