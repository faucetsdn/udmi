...
from triage.impl.resolver import UDMILogResolver
...
def evaluate_single_directory(bundles_path, target, output_dir, analyzer):
    ...
    if run_dirs:
        print(
            f"Found {len(run_dirs)} existing run directories. Skipping zip/tgz extraction.")
        ...
        if zips:
            ...
            for i, zip_path in enumerate(zips, start=1):
                ...
                cmd_status = run_command(["mantis/bin/support_process", zip_path])
                if cmd_status != 0:
                    print(f"Warning: support_process failed for {zip_path}", file=sys.stderr)
                ...
                if analysis:
                    for ukey, val in analysis.items():
                        if val['outcome'] == 'fail':
                            ...
                            # Force dynamic active_site inside resolver
            ...
            cmd_status = run_command(["mantis/bin/support_process", bundles_path])
            if cmd_status != 0:
                print(f"Warning: support_process failed for {bundles_path}", file=sys.stderr)
