...
3. **`diagnose` (Stage 3)** 🦗👁️: An AI-powered diagnostic triage agent that ingests the manifest, correlates sharded execution logs using padded execution timebounds, crawls codebase repositories/git histories, executes progressive multi-stage playbook tasks, and generates rich defect triage reports.
...
├── docs/
│   ├── evaluate_stability_spec.md  # Spec for Stability Evaluator (Stage 2)
│   ├── diagnose_spec.md      # Spec for AI Diagnostics Triage (Stage 3)
│   ├── telemetry_correlation_guide.md  # Chronological log correlation & RCA guide
│   └── generic_triage_harness_guide.md  # Guide for adapting the AI Harness to non-UDMI projects
...
└── src/                      # Source code
    ...
    └── triage/               # Submodule 3: Triage Agent (Stage 3)
        ├── harness/          # Generic triage pipeline & agent engine
        └── impl/             # UDMI-specific playbook & tools implementations
```
