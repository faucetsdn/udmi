...
## scan_depth_buckets+vendor (ALPHA)

Check results of a single scan with buckets depth
...
1. Update config before scheduled scan active
    * Add `discovery.families.vendor` = { "generation": `family generation`, "depth": `buckets`, "scan_duration_sec": `10` }
...
1. Check that no scan addresses present in buckets scan
1. Check that all scan networks are unique
