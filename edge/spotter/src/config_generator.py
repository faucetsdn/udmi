...
def parse_project_spec(project_spec, site_config):
    raw_spec = project_spec[2:] if project_spec.startswith("//") else project_spec
...
def generate_spotter_config(site_path, project_spec, device_id, out_path=None, metrics_rate_sec=300):
    ...
    if not os.path.isfile(site_config_path):
        raise ValueError(f"cloud_iot_config.json not found inside site path: {site_config_path}")
    ...
    base_key, base_cert = find_key_cert(device_dir)
    if not base_key:
...
def main():
    parser = argparse.ArgumentParser(description="Generate dynamic Spotter runtime configuration from site model.")
