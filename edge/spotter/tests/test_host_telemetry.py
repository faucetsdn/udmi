...
from edge.spotter.src.host_telemetry import get_host_hardware_info
from edge.spotter.src.host_telemetry import get_host_os_info
...
class TestHostTelemetry(unittest.TestCase):
  ...
  def test_get_host_hardware_info(self, mock_exists):
    """Verifies DMI sysfs reading of host make and model."""
    mock_exists.side_effect = lambda p: p in (
        "/sys/class/dmi/id/sys_vendor",
        "/sys/class/dmi/id/product_name",
    )

    def custom_open(path, *_args, **_kwargs):
  ...
  def test_get_host_os_info(self, mock_exists):
    """Verifies parsing of os-release key-value format."""
...
"""
    with patch("builtins.open", mock_open(read_data=os_release_data)):
      ...
      self.assertEqual(
          info.get("PRETTY_NAME"), "Debian GNU/Linux 12 (bookworm)"
      )

  @patch("os.path.exists")
  def test_get_cpu_and_memory_metrics(self, mock_exists):
    """Verifies parsing of /proc/meminfo and /proc/loadavg."""
...
"""
  ...
  def test_safety_circuit_breaker(self, mock_metrics):
    """Verifies circuit breaker tripping when threshold is exceeded."""
