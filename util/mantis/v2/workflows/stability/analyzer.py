...
class TestExecutionKey:
    """Type-safe key representing a single test execution instance across runs."""
    suite: str
    type: str
    category: str
    test_name: str
    ...
    def id(self) -> str:
        device_prefix = f"{self.device_id}:" if self.device_id else ""
        return f"{device_prefix}{self.suite}:{self.type}:{self.category}:{self.test_name}:{self.occurrence_index}"

    @classmethod
    def from_id(cls, key_id: str) -> "TestExecutionKey":
        parts = key_id.split(":")
        # Format can be "device_id:suite:type:category:test_name:idx" or "suite:type:category:test_name:idx"
        if len(parts) == 6:
            return cls(
                device_id=parts[0],
                suite=parts[1],
            ...
            return cls(
                ...
                category=parts[2],
                test_name=parts[3],
                occurrence_index=int(parts[4])
            )
        else:
            raise ValueError(f"Invalid unique key id format: {key_id}")


from mantis.tools.resolver import UDMIResultParser


def normalize_text(text):
    """Normalize dynamic variables (timestamps, message details) in output strings."""
    return UDMIResultParser.normalize_reason(text)
...
class TestResult:
    def __init__(self, raw_line, device_id=None, is_itemized=False):
        ...
        if is_itemized:
            ...
            if tokens and (
                tokens[0].isdigit() or re.match(r'^\d+$', tokens[0])):
                tokens = tokens[1:]
        ...
        self.reason = normalize_text(" ".join(tokens[6:])) if len(
            tokens) > 6 else ""
...
class GoldenBaseline:
    ...
    def load(self):
        if not os.path.exists(self.filepath):
            print(f"Warning: Golden baseline file not found at {self.filepath}",
                  file=sys.stderr)
            return
...
class RunAnalyzer:
    def __init__(self, udmi_root="."):
        ...
        self.sequencer_baseline = GoldenBaseline(
            os.path.join(udmi_root, "etc/sequencer.out"), is_itemized=False)
        self.itemized_baseline = GoldenBaseline(
            os.path.join(udmi_root, "etc/test_itemized.out"), is_itemized=True)
    ...
    def analyze_run(self, sequencer_out_path=None, itemized_out_path=None):
        ...
        if sequencer_out_path and os.path.exists(sequencer_out_path):
            results = self.parse_results_file(sequencer_out_path,
                                              is_itemized=False)
            analysis.update(
                self.analyze_results(results, test_suite='sequencer'))

        if itemized_out_path and os.path.exists(itemized_out_path):
            results = self.parse_results_file(itemized_out_path,
                                              is_itemized=True)
            analysis.update(
                self.analyze_results(results, test_suite='itemized'))
    ...
    def aggregate_runs(self, runs_analyses):
        ...
        for ukey in all_keys:
            ...
            # Pass rate measures stability (how often it behaved exactly as expected out of all runs where it was present)
            runs_present = pass_count + fail_count
            pass_rate = (pass_count / runs_present) * 100 if runs_present > 0 else 0.0
