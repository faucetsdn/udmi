    ...
    try:
        ...
        if pids_to_kill:
            ...
            for pid in pids_to_kill:
                subprocess.run(["kill", "-9", pid], stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL)
    except Exception as e:
        print(f"Warning: failed to clean pubber processes: {e}",
              file=sys.stderr)
...
class GitHubClient:
    ...
    def _request(self, method, path, data=None):
        ...
        req = urllib.request.Request(url, data=req_data, headers=self.headers,
                                     method=method)
        try:
    ...
    def get_completed_runs(self, branch, limit=10):
        """Fetches already executed completed workflow runs on a specific branch for testing.yml."""
        path = f"/repos/{self.owner}/{self.repo}/actions/workflows/testing.yml/runs?branch={branch}&status=completed&per_page={limit}"
        res = self._request("GET", path)
        runs = []
        if res and res.get("workflow_runs"):
            for run in res["workflow_runs"]:
    ...
    def get_run_artifact_url(self, run_id):
        ...
        if res and res.get("artifacts"):
            for art in res["artifacts"]:
                if art["name"].startswith("udmi-support_") and not art.get(
                    "expired", False):
                    return art[
                        "id"], f"/repos/{self.owner}/{self.repo}/actions/artifacts/{art['id']}/zip"
        return None, None
    ...
    def download_artifact(self, download_path, save_filepath):
        ...
        try:
            ...
            print(f"Error downloading artifact from {url}: {e}",
                  file=sys.stderr)
            return False


def run_local_sandbox(args, iterations, output_dir):
    """Executes the local sandbox running loop (Mode 1)."""
    print(f"=============================================================")
    ...
    for i in range(1, iterations + 1):
        print(
            f"\n=============================================================")
        print(
            f"🚀 Starting Local Iteration {i} of {iterations} at {datetime.now().strftime('%H:%M:%S')}")
        print(
            f"=============================================================")
        ...
        if args.suite in ["sequencer", "both"]:
            ...
            run_command(["bin/test_sequencer", "clean", "nocheck",
                         args.target] + specific_tests)
            run_command(["bin/test_sequencer", "alpha", "nocheck",
                         args.target] + specific_tests)
        ...
        # 4. Pack raw outcomes and test logs into standard udmi-support_run_{idx}.tgz tarball
        tgz_filename = f"udmi-support_run_{i}.tgz"
        tgz_filepath = os.path.join(output_dir, tgz_filename)

        for old_support in glob.glob(
            os.path.join(UDMI_ROOT, "*udmi-support*.tgz")):
            try:
                os.remove(old_support)
            except Exception:
                pass
        ...
        if support_rc == 0:
            ...
            if support_archives:
                ...
                try:
                    ...
                    print(
                        f"Successfully packaged and saved core support archive: {tgz_filepath}")
                except Exception as e:
                    print(f"Error moving support archive: {e}",
                          file=sys.stderr)
        clean_pubber_processes()
...
def run_github_ci_mode(args, runs, output_dir, token, owner, repo):
    ...
    if args.mode == "ci_search":
        ...
        print(
            f"Discovered {len(new_run_ids)} completely executed runs: {', '.join(map(str, new_run_ids))}")
    ...
    # SUB-MODE B: DISPATCH & POLL NEW WORKFLOW RUNS
        ...
        for idx in range(1, iterations + 1):
            try:
                ...
                print(f"Failed to trigger run dispatch {idx}, aborting.",
                      file=sys.stderr)
                sys.exit(1)
        ...
        if len(new_run_ids) < iterations:
            print(
                f"Warning: Only resolved {len(new_run_ids)} out of {iterations} runs. Polling remainder.",
                file=sys.stderr)
        else:
            print(
                f"Successfully resolved run IDs: {', '.join(map(str, new_run_ids))}")
        ...
        while len(completed_runs) < len(new_run_ids):
            ...
            print(
                f"  [{datetime.now().strftime('%H:%M:%S')}] Queued: {queued_count} | In-Progress: {in_progress_count} | Success: {success_count} | Failed: {failed_count} | Cancelled: {cancelled_count}",
                end="\r")
    ...
    for run_id in sorted(new_run_ids):
        ...
        if art_path:
            ...
            print(
                f"  ⚠️ Warning: No support artifact found for Completed Run {run_id}.",
                file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--iterations", type=int,
                        help=argparse.SUPPRESS)  # Hidden legacy alias for compatibility
    parser.add_argument("--mode", "-m", choices=["local", "ci", "ci_search"],
                        default="local",
    ...
    parser.add_argument("--suite", choices=["sequencer", "itemized", "both"],
                        default="both",
                        help="Test suite to run locally (default: both)")
    parser.add_argument("--tests",
                        help="Comma-separated list of selective sequencer tests to run locally (e.g. valid_serial_no)")
    ...
    # Dynamic Argument Warnings
    if args.mode in ["ci", "ci_search"]:
        if args.tests is not None or args.suite != "both" or args.verbose:
            print("⚠️ Warning: --suite, --tests, and --verbose are only applicable in 'local' mode and will be ignored.", file=sys.stderr)

    if args.mode == "ci_search" and args.target != "//mqtt/localhost":
        parser.error("--target / -t option is not supported in 'ci_search' mode.")
    ...
    if args.mode == "local":
        run_local_sandbox(args, runs, output_dir)
    else:
        # Get GitHub Token
        ...
        if not token:
            print("Error: The environment variable 'GITHUB_TOKEN' is not set.",
                  file=sys.stderr)
            print(
                "Please set your GitHub Personal Access Token (PAT) before executing.",
                file=sys.stderr)
            print("Example: export GITHUB_TOKEN=\"your_pat_here\"",
                  file=sys.stderr)
            sys.exit(1)
        ...
        run_github_ci_mode(args, runs, output_dir, token, owner, repo)
