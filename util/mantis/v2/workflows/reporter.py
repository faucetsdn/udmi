...
class UDMITriageReporter:
    ...
    def __init__(self, target: str, site_id: str, out_dir: str, failure_classifiers: Dict[str, str] = None):
        self.target = target
        self.site_id = site_id
        self.out_dir = out_dir
        self.failure_classifiers = failure_classifiers
    ...
    def compile_summary_report(self, triage_summaries: List[Dict[str, Any]]) -> str:
        ...
        for s in triage_summaries:
            ...
            classified = False
            if self.failure_classifiers:
                import sys
                for label, pattern in self.failure_classifiers.items():
                    try:
                        if re.search(pattern, bp, re.IGNORECASE):
                            sig = label
                            classified = True
                            break
                    except re.error as e:
                        print(f"Warning: Invalid regular expression classifier pattern '{pattern}' for category '{label}': {e}", file=sys.stderr)
