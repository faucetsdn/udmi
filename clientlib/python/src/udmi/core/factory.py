...
wire them together. It acts as the Composition Root for the application.
"""
...
def get_default_managers(**kwargs) -> List[BaseManager]:
    ...
    Args:
        **kwargs:
            ...
            points_bulk_provider (BulkPointProvider): Optional provider for bulk telemetry reads.
    """
    ...
    points_bulk_provider = kwargs.get('points_bulk_provider')
...
def _create_dispatcher_stack(
    ...
    """
    Factory function matching the Device.ConnectionFactory signature (partially).
    Builds a fresh Client -> Dispatcher stack for connection resets.
    """
...
def create_device(
    ...
    This function orchestrates the creation of a fully functional UDMI Device.
    It automatically detects the authentication strategy (mTLS vs JWT) based
    on the provided config and key file.

    Args:
        endpoint_config: Connection params (host, port, client_id).
        managers: Custom managers list. Defaults to standard set if None.
        client_config: TLS and Reconnect settings.
        key_file: Path to private key (for JWT/mTLS).
        persistence_path: File path for state storage.
        initial_model: Metadata model to seed state.
        persistence_backend: Custom persistence backend override.
    ...
    client = create_client_from_endpoint_config(
        ...
        client_config.reconnect_config,
        credential_manager=credential_manager
    )
