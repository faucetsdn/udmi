...
spec = util.spec_from_loader("sequence_report", machinery.SourceFileLoader("sequence_report", "bin/sequencer_report"))
...
class TestSequencerReport(unittest.TestCase):
  ...
  def test_class_single(self):
    ref = [['Step 1'], ['Step 2'], ['Step 3'], [' Step 4']]
    done, fail, todo = seq.Sequence.classify_steps(ref, ref[0], 'fail')
  ...
  def test_formatter_real(self):
    ref = seq.Sequence.get_steps(REAL_SEQUENCE)
    act = seq.Sequence.get_steps(REAL_SEQUENCE_PARTIAL)

    a = seq.Sequence.format(ref, act, SampleTestResult())
    
    #print('\n'.join(a))
        
  def test_sequence(self):
    output = seq.Sequence(SampleTestResult, 'validator/sequences', 'sites/udmi_site_model/out/devices/AHU-1/tests')
