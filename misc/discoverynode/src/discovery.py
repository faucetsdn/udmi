...
class DiscoveryController(abc.ABC):
  ...
  def start_discovery(self):
    ...
    This method should:
      - Setup and clear any required variables, e.g. dictionaries, memory/previously remembered results
  ...
  def __init__(self, state: schema.state.State, publisher: Callable):
    self.state = schema.state.Discovery()
  ...
  def _status_from_exception(self, err: Exception) -> None:
    ...
    self.state.status = schema.state.Status(category="discovery.error", level=500, message=str(err))
  ...
  def controller(self, config_dict: None | dict[str: Any]):
    ...
    with self.mutex:
      discovery_config_dict = config_dict["discovery"]["families"].get(self.scan_family)
      config = schema.config.DiscoveryFamily(**discovery_config_dict) if discovery_config_dict else None
