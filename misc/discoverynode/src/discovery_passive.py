...
class PassiveNetworkDiscovery(discovery.DiscoveryController):
  ...
  def __init__(self, state, publisher):
    ...
    # Configure Scapy
    scapy.all.conf.layers.filter([scapy.layers.inet.Ether, scapy.layers.inet.IP, scapy.layers.inet.ICMP])
  ...
  def start_discovery(self):
    ...
    self.queue_thread = threading.Thread(target=self.queue_worker, args=[], daemon=True)
    self.queue_thread.start()

    # Turn-on the main service and publisher
    self.service_thread = threading.Thread(target=self.discovery_service, args=[], daemon=True)
  ...
  def discovery_service(self):    
    while True:      
      ...
      for device_record in new_device_records:
        self.publisher(schema.discovery_event.DiscoveryEvent(
            generation=self.config.generation,
            scan_addr=device_record.addr,
            scan_family=self.scan_family,
            families=dict(
                  ethmac=schema.discovery_event.DiscoveryFamily(device_record.mac)
                )
        ))
      ...
      try:
        ...
        if scapy.layers.inet.IP in item :
          ...
          for x in ["src", "dst"]:
            ...
            if (ip := getattr(item[scapy.layers.inet.IP], x)) and ip is not None and ip not in self.addresses_seen:
              if ipaddress.ip_address(ip).is_private:
                self.addresses_seen.add(ip)
                self.device_records.add(PassiveScanRecord(
                    addr=ip,
