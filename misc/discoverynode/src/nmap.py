...
class NmapPort:
  ...
  Attributes:
    ...
    open    (bool) if port is considered open
  """
  port_number: int
...
def results_reader(self, nmap_file:str) -> Generator[NmapHost, None, None]:
  """Load NMAP Results Reader

  Args:
    nmap_file  path to NMAP file output (generated with -xO argument)
  ...
  for _host in root.iter('host'):
    ...
    for _port in _host.iter('port'):
      ...
      if (script := _port.find('script')) is not None and script.attrib['id'] == "banner":
